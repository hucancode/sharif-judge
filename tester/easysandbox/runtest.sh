...
diff_rc=$?
actual_output=`cat ${actual} -v`
expected_output=`cat ${expected} -v`
rm -f ${actual}
if [ $diff_rc != 0 ]; then
	echo "failed (output mismatch, expected [${expected_output}], got [${actual_output}])"
	exit 1
fi
