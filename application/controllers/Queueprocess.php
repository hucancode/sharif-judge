...
class Queueprocess extends CI_Controller
{
	...
	public function run()
	{
		//file_put_contents("/home/hu/Desktop/log.txt","queue process run\r\n",FILE_APPEND|LOCK_EX);
		...
		do { // loop over queue items
			...
			$userid = $this->user_model->username_to_user_id($username);
			...
			//file_put_contents("/home/hu/Desktop/log.txt","load queue item submit id=".$submit_id." username=".$username."\n",FILE_APPEND|LOCK_EX);
			$submission = $this->submit_model->get_submission($assignment, $submit_id);
			//file_put_contents("/home/hu/Desktop/log.txt","load submission ".$submission['file_name']."\n",FILE_APPEND|LOCK_EX);
			...
			elseif ($file_type === 'py2' || $file_type === 'py3')
			...
			$cmd = "cd $tester_path;\n./tester.sh $problemdir ".escapeshellarg($username).' '.escapeshellarg($raw_filename)." $file_type $time_limit $time_limit_int $memory_limit $output_size_limit $diff_cmd $diff_arg $op1 $op2 $op3 $op4 $op5";
			...
			//file_put_contents("/home/hu/Desktop/log.txt","remove item\r\n",FILE_APPEND|LOCK_EX);
